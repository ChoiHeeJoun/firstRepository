<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" 
    import="com.kh.jsp.notice.model.vo.*"%>
<%
		...
		<form id="updateForm" method="POST">
			...
			<script>
				function complete(){
					$('#updateForm').attr(
							"action", 
							"<%=request.getContextPath()%>/update.no");
				}
				
				function deleteNotice(){
					$('#updateForm').attr(
							"action", 
							"<%=request.getContextPath()%>/deleteNotice.no");
				}
			</script>
		</form>
	</div>
